...
public class Tirer extends Action {
    ...
    public void executer() {
        if (this.executeur instanceof Marshall) {
            System.out.println("tir marshall pas encore traité");
        }
            ...
            switch (this.direction) {
                case Droite:
                    ...
                    if (this.executeur.getEmplacement() instanceof Locomotive || (this.executeur.getEmplacement() instanceof Toit) && ((Toit) this.executeur.getEmplacement()).getCabine() instanceof Locomotive) {
                        ...
                        while (toitCourant.getBanditList((Bandit)this.executeur).isEmpty()){
                            toitCourant = (Toit) toitCourant.getVoisin(Direction.Droite);
                        }
                        if (!toitCourant.getBanditList((Bandit) this.executeur).isEmpty()) {
                                Bandit banditBlesse = toitCourant.getBanditAlea((Bandit) this.executeur);
                                if (!banditBlesse.getButtins().isEmpty()) {
                                    Buttin buttinPerdu = banditBlesse.retirerButtin();
                                    toitCourant.ajouterButin(buttinPerdu);
                                    System.out.println(this.executeur.getSurnom() + " à tirer à droite sur le toit");
                                }
                            }
                        }
                    }
                        ...
                        if (this.executeur.getEmplacement() instanceof Interieur) {
                            ...
                            if (!toitCourant.getBanditList((Bandit) this.executeur).isEmpty()) {
                                ...
                                if (!banditBlesse.getButtins().isEmpty()) {
                                    Buttin buttinPerdu = banditBlesse.retirerButtin();
                                    toitCourant.ajouterButin(buttinPerdu);
                                    System.out.println(this.executeur.getSurnom() + " à tirer à gauche sur le toit");
                                }
                            }
                        }
                    }
